...
const routes = [
  {
    ...
    keywords: 'Batik Sri Lanka, Bathik Sri Lanka, bathik price in Sri Lanka, batik sarees Sri Lanka, batik shirts Sri Lanka, Kesara Bathik',
    ...
    sections: [
      ['Shop Sri Lankan batik online', 'Browse category pages for women, men, kids, family kits and accessories. Each active product includes photographs, product information, price and availability.'],
      ['Batik and bathik prices in Sri Lanka', 'Sri Lankan visitors see separately maintained LKR product prices. Overseas customers can browse the supported international website currencies.'],
    ],
    faqs: [
      ['What is the difference between batik and bathik?', 'Batik is the standard English spelling, while bathik is a commonly used alternative spelling in Sri Lanka. Both terms are used on this website to help customers find the same handcrafted clothing.'],
      ['Where can I see current bathik prices?', 'Open the products or category pages and then select an individual item to view its current price and availability.'],
    ],
  },
  {
    path: '/products',
    ...
    keywords: 'batik clothing Sri Lanka, bathik price in Sri Lanka, buy batik online Sri Lanka',
    ...
    sections: [
      ['Find the correct collection', 'Use the women, men, kids, family kits and accessories pages to reach focused collections with their own descriptions and product listings.'],
      ['Product information', 'Individual product pages provide images, descriptions, available options, current prices and stock information.'],
    ],
    faqs: [
      ['Can I order batik products online?', 'Yes. Add available products to the cart and continue through checkout using the delivery and payment options shown on the website.'],
    ],
  },
  {
    path: '/new-arrivals',
    ...
    keywords: 'new batik designs Sri Lanka, latest bathik sarees, new batik shirts',
    ...
    newArrival: true,
    sections: [
      ['Recently added designs', 'Active products marked as new arrivals are collected here so customers can quickly find the latest available designs.'],
    ],
    faqs: [],
  },
  {
    ...
    keywords: 'batik sarees Sri Lanka, bathik saree price Sri Lanka, Kandyan batik saree, women bathik Sri Lanka',
    ...
    category: 'Women',
    sections: [
      ['Sri Lankan batik sarees and women’s clothing', 'This collection brings together active women’s sarees, Kandyan-inspired designs, frocks, kaftans, tops and kurtha sets. Product pages show the latest images, prices and availability.'],
      ['Bathik saree prices in Sri Lanka', 'Sri Lankan visitors can view the separately maintained LKR price on each product page. Prices and available options may differ between designs.'],
      ['Ordering and delivery', 'Compare the product photographs and descriptions, add available items to the cart and provide delivery details during checkout.'],
    ],
    faqs: [
      ['Where can I view bathik saree prices in Sri Lanka?', 'Open an individual women’s product page to see its current LKR price, images and availability.'],
      ['Can overseas customers order Sri Lankan batik sarees?', 'Yes. Overseas customers can browse supported currencies and complete delivery details at checkout.'],
    ],
  },
  {
    path: '/men',
    ...
    keywords: 'batik shirts Sri Lanka, men bathik shirts, batik sarong Sri Lanka, Avurudu batik kits',
    ...
    category: 'Men',
    sections: [
      ['Men’s Sri Lankan batik clothing', 'The men’s collection is dedicated to active batik shirts, traditional sarongs and Avurudu clothing. Older products stored under a recognised men’s subcategory are also included.'],
      ['Current prices and availability', 'Open an individual product page for its latest photographs, description, price, available options and stock information.'],
      ['A permanent men’s batik guide', 'Even while individual designs are being updated, this page continues to provide useful men’s collection information and links to the full store instead of behaving like a missing page.'],
    ],
    faqs: [
      ['What products appear in the men’s batik collection?', 'Active batik shirts, men’s sarongs and Avurudu kits are grouped on this page.'],
      ['How can I check the current bathik price?', 'Open the individual product listing because prices and available options can differ by design.'],
    ],
  },
  {
    path: '/kids',
    ...
    keywords: 'kids batik Sri Lanka, bathik frocks for kids, kids batik shirts, lama saree batik',
    ...
    category: 'Kids',
    sections: [
      ['Colourful kids’ batik designs', 'The collection groups active children’s frocks, lama sarees, shirts and sarong sets under one dedicated category URL.'],
      ['Prices and product details', 'Each product page provides its current price, images, available options and stock information.'],
    ],
    faqs: [
      ['Are new kids’ designs added here?', 'Active kids’ products appear automatically when they are added to the online store.'],
    ],
  },
  {
    path: '/family-kits',
    ...
    keywords: 'batik family kits Sri Lanka, matching bathik clothes, Avurudu family batik',
    ...
    sections: [
      ['Coordinated family clothing', 'Family kits bring related colours and patterns together for celebrations and photographs.'],
      ['Confirm each set before ordering', 'Check the individual product page for the latest images, included pieces, available options and price.'],
    ],
    faqs: [
  ...
  },
  {
    ...
    keywords: 'batik accessories Sri Lanka, batik bags Sri Lanka, bathik clutch, handmade accessories',
    ...
    sections: [
      ['Batik accessories for complete outfits', 'This category separates active bags, clutches, jewellery, slippers and hair accessories from the clothing collections.'],
      ['Product-specific details', 'Review each product listing for its latest images, description, price and availability.'],
    ],
    faqs: [
      ['Which batik accessories are available?', 'Active products can include bags, clutches, jewellery, slippers and hair accessories.'],
    ],
  },
  {
    path: '/faq',
    ...
    keywords: 'Sri Lankan batik FAQ, bathik prices, batik shipping, batik online order',
    ...
    sections: [
      ['Shopping information', 'Use the FAQ page together with individual product details and the website policies when preparing an order.'],
    ],
    faqs: [],
  },
]
...
function selectedProducts(route, products) {
  ...
  if (route.newArrival) selected = selected.filter((product) => Boolean(product.isNewArrival || product.newArrival))
...
function staticSnapshot(route, products) {
  ...
  const sections = route.sections.map(([heading, text]) => `<section><h2>${escapeHtml(heading)}</h2><p>${escapeHtml(text)}</p></section>`).join('')
...
for (const route of routes) {
  ...
  html = html.replace('</head>', `  <link rel="sitemap" type="application/xml" href="${SITE_URL}/sitemap.xml" />\n  ${snapshotStyle}\n  <script type="application/ld+json">${JSON.stringify(routeSchema(route, products))}</script>\n</head>`)
  html = html.replace('<div id="root"></div>', `<div id="root" data-prerendered="true">${staticSnapshot(route, products)}</div>`)

  const outputFile = route.path === '/' ? 'index.html' : `${route.path.slice(1)}.html`
  await writeFile(path.join(distDir, outputFile), html, 'utf8')
}

console.log(`[prerender] Wrote ${routes.length} clean route snapshots using ${products.length} products`)
