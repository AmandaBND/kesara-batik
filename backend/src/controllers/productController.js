...
function parseFormData(data) {
  ...
  if (typeof data.metaKeywords === 'string') {
    try {
      ...
      data.metaKeywords = data.metaKeywords.split(',').map(t => t.trim()).filter(Boolean);
    }
  }
...
function sanitizeUpdateData(data) {
  const blocked = ['_id', 'slug', 'rating', 'reviewCount', 'soldCount', 'createdAt', 'updatedAt', '__v'];
...
exports.getProducts = asyncHandler(async (req, res) => {
  const { category, parentCategory, search, sort, minPrice, maxPrice, page = 1, limit = 12, featured, newArrival, trending, showAll } = req.query;
  const query = {};
